...
function putRow($row, $row_num)
{
    echo '<tr>';
    foreach ($row as $key => $value) {
        echo "<td class='text-left' onclick='letsManage(\"{$row['cid']}\")' style='cursor: pointer'>" . $value . "</td>";
    }
    echo "<td class='text-left' onclick='letsManage(\"{$row['cid']}\")' style='cursor: pointer'>" . $row_num['num'] . "</td>";
    echo '</tr>';
}
...
{
    $qy = "SELECT COUNT(*) AS num FROM chosedcourse WHERE cid = '{$cid}'";
    $rec_num = $lk->query($qy);
    return $rec_num->fetch_array(MYSQLI_ASSOC);
}

function putTable($rec, $lk)
{
    echo "<form method='post' id='manageForm' action='manageEvent.php'>";
    ...
    if ($row = $rec->fetch_array(MYSQLI_ASSOC)) {
        ...
        putRow($row, get_chose_num($row['cid'], $lk));
    ...
    <main>
        <div class="toolbar">
            ...
            <div class="search bar">
                <form class="form1" action="fuzzsearch.php">
                    <input type="text" name="fuzz" placeholder="请输入您要搜索课程名">
        ...
        <div>
            ...
            if ($fu == null) {
                echo "<script>";
                echo "window.location.replace('./teacherManage.php');";
                echo "</script>";
            }

            $rec = $connect->query("SELECT cid, cname, credit FROM course WHERE cname  LIKE '%{$fu}%' AND tid=$uid");
            putTable($rec, $connect);
            ?>
        </div>
    </main>
    <sidebar>
